...
def ini_to_df_r(root):
    ...
    for ini in root.findall(".//pt_gov_ar_objectos_iniciativas_DetalhePesquisaIniciativasOut"):
        for voting in ini.findall('.//pt_gov_ar_objectos_VotacaoOut'):
            votep = voting.find('./detalhe')
            if votep is not None:
                init_dict = collections.OrderedDict()
                counter +=1                 
                init_dict['id'] = voting.find('id').text
                init_dict['iniTitulo'] = ini.find('iniTitulo').text
...
l14_ini_df = ini_to_df_r(l14_ini_tree)
l14_ini_df
...
votes = l14_ini_df
votes = votes.loc[votes['data'] > "2021-10-27"]
parties = l14_parties
...
with pd.option_context("display.max_columns", 0):
    display(votes[["resultado", "id","data"] + parties])
...
sns.clustermap(
    ...
    row_linkage=distmat_link,
    col_linkage=distmat_link,
    figsize=(12,12)).fig.suptitle('Portuguese Parliament 14th Legislature, Clustermap',y=1)
plt.show()
