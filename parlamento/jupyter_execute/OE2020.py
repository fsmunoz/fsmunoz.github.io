...
# O Orçamento de Estado para 2020 foi votado em Fevereiro de 2020 e representa um ponto importante para a análise não só das legislaturas, mas também do posicionamento dos partidos nas votações de orçamentos seguintes: desde logo o para 2021, analisado de seguida, e que seria o último Orçamento de Estado viabilizado ao governo minoritário do PS.
# 
# Estas votações não estão incluídas na análise anterior: os dados utilizados relativos a votações no Parlamento não incluem, por exemplo, as votações feitas em Comissões. Esta actualização aborda um conjunto de votações independente, muito mais circunscrito no tempo e (dentro da grande variedade de temas abordados no OE) no conteúdo.
# 
# ```{margin}
# «"O Parlamento aprovou esta quinta-feira, em votação final global, a proposta de Orçamento do Estado para 2020. O documento passou com os votos a favor da bancada do PS, os votos contra de PSD, CDS-PP, Iniciativa Liberal e Chega e as abstenções de BE, PCP, "Os Verdes", PAN e da deputada Joacine Katar Moreira.», {cite}`portugalAprovadoOrcamentoEstado`.
# ```
# Este Orçamento foi viabilizado com a abstenção dos partidos à esquerda do PS - o último a sê-lo por todos os que inicialmente suportaram o governo minoritário do PS.
...
oe_url = "https://app.parlamento.pt/webutils/docs/doc.xml?path=&fich=OEPropostasAlteracao2019Or.xml&Inline=true"
#oe_url="https://app.parlamento.pt/webutils/docs/doc.xml?path=&fich=OEPropostasAlteracao2020Or.xml&Inline=true"
...
# A quantidade de propostas de BE, PCP e CDS-PP é, no limite superior, bastante próxima. Esta situação modifica-se quando analisamos as propostas aprovadas: se o PS vê a quase totalidade das suas propostas aprovadas, e o PCP consegue um número ligeiramente maior de aprovações que o BE, já o CDS-PP tem a maior parte das suas propostas rejeitadas.
...
# 
# * O maior número de propostas aprovadas é do partido do Governo, o PS, com uma percentagem de aprovação acima dos 90%.
# * O BE tem um número de proposta marginalmente suporior ao PCP, que por sua vez tem um número de proposta aprovadas também marginalmente superior ao BE.
# * O PAN e PSD têm um número de propostas totais aproximado, e ambos com uma taxa de sucesso reduzido.
# * O CDS-PP é o mais penalizado em termos do rácio propostas/aprovações.
# * O PEV é, a seguir ao PS, o com melhor rácio de aprovações.
# 
# 
# ### Propostas aprovadas
# 
# E que propostas cada partido conseguiu aprovar? A tabela seguinte indica o número total e os detalhes de cada uma.
...
mycol  = ['GP', 'BE', 'PCP', 'PEV','PS', 'PAN', 'PSD','CDS-PP' ]
...
import numpy as np
votes_hm = oe_dfr[['BE', 'PCP' , 'PS', 'PSD','CDS-PP']]
...
for area in oe_df["Domain"].unique():
    ...
    avotes_hm = varea[['BE', 'PCP' , 'PS', 'PAN', 'PSD','CDS-PP']]
